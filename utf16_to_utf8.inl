...
#endif
static size_t utf16_to_utf8_z(const UTF16CHAR **const w, UTF8CHAR **const b, size_t sz)
{
...
/* convert 'n' UTF16CHARs to utf8 ones,
 input:
  ...
  b  - optional address of the pointer to the beginning of output buffer,
  sz - free space in output buffer, in UTF8CHARs, if zero - output buffer is not used,
  n  - number of UTF16CHARs to convert, if zero - input and output buffers are not used.
...
#endif
static size_t utf16_to_utf8(const UTF16CHAR **const w, UTF8CHAR **const b, size_t sz, const size_t n)
{
	if (n) {
		/* unsigned integer type must be at least of 32 bits */
		size_t m = 0*sizeof(int(*)[1-2*((unsigned)-1 < 0xFFFFFFFF)]);
		  ...
		  3 - at least two UTF16CHARs (4 bytes) were checked, no overflow of 'm' is possible */
		{
			...
			if (sz < m)
				goto too_long;
#endif
			*w = t; /* points after the last successfully converted UTF16CHAR, (*w) < se */
			return sz; /* ok, >0 */
		}
too_long:
		*w = s; /* (*w) == se */
...
#endif
static size_t utf16_to_utf8_z_unsafe_out(const UTF16CHAR *w, UTF8CHAR *const buf)
{
	/* unsigned integer type must be at least of 32 bits */
...
A_At(buf, A_Post_z A_Post_readable_size(return))
#endif
static size_t utf16_to_utf8_z_unsafe(const UTF16CHAR *w, UTF8CHAR *const buf)
{
	/* unsigned integer type must be at least of 32 bits */
	UTF8CHAR *b = buf + 0*sizeof(int(*)[1-2*((unsigned)-1 < 0xFFFFFFFF)]);
	for (;;) {
...
/* convert 'n' UTF16CHARs to utf8 ones, (if 'n' is zero - input and output buffers are not used),
  do not check if there is enough space in output buffer, assume it is large enough.
 returns number of stored UTF8CHARs:
...
#endif
static size_t utf16_to_utf8_unsafe_out(const UTF16CHAR *w, UTF8CHAR *const buf, const size_t n)
{
	if (n) {
		/* unsigned integer type must be at least of 32 bits */
		UTF8CHAR *b = buf + 0*sizeof(int(*)[1-2*((unsigned)-1 < 0xFFFFFFFF)]);
...
A_When(n, A_At(buf, A_Out A_Post_readable_size(return)))
#endif
static size_t utf16_to_utf8_unsafe(const UTF16CHAR *w, UTF8CHAR *const buf, const size_t n)
{
	if (n) {
		/* unsigned integer type must be at least of 32 bits */
		UTF8CHAR *b = buf + 0*sizeof(int(*)[1-2*((unsigned)-1 < 0xFFFFFFFF)]);
